...
import net.vixiv.instant.BuildConfig;
...
import net.vixiv.instant.adblock.AdBlock;
...
public class InstantWebClient extends WebViewClient {

    private static final String TAG = "InstantWebClient";

    @NonNull private final Activity mActivity;
    @NonNull private final InstantView mInstantView;
    ...
    InstantWebClient(@NonNull Activity activity, @NonNull InstantView instantView) {
        BrowserApp.getAppComponent().inject(this);
        Preconditions.checkNonNull(activity);
        Preconditions.checkNonNull(instantView);
        mActivity = activity;
        mUIController = (UIController) activity;
        mInstantView = instantView;
    ...
    public void onPageFinished(@NonNull WebView view, String url) {
        ...
        if (view.getTitle() == null || view.getTitle().isEmpty()) {
            mInstantView.getTitleInfo().setTitle(mActivity.getString(R.string.untitled));
        } else {
            mInstantView.getTitleInfo().setTitle(view.getTitle());
        }
        if (Build.VERSION.SDK_INT >= android.os.Build.VERSION_CODES.KITKAT &&
            mInstantView.getInvertePage()) {
            view.evaluateJavascript(Constants.JAVASCRIPT_INVERT_PAGE, null);
        }
        mUIController.tabChanged(mInstantView);
    }

    @Override
    public void onPageStarted(WebView view, String url, Bitmap favicon) {
        mInstantView.getTitleInfo().setFavicon(null);
        if (mInstantView.isShown()) {
    ...
    public void onScaleChanged(@NonNull final WebView view, final float oldScale, final float newScale) {
        if (view.isShown() && mInstantView.mPreferences.getTextReflowEnabled() &&
    ...
    private boolean shouldOverrideLoading(@NonNull WebView view, @NonNull String url) {
        ...
        Map<String, String> headers = mInstantView.getRequestHeaders();

        if (mInstantView.isIncognito()) {
        ...
        if (url.startsWith("mailto:")) {
            ...
            if (file.exists()) {
                ...
                try {
                    ...
                    System.out.println("InstantWebClient: cannot open downloaded file");
                }
            } else {
