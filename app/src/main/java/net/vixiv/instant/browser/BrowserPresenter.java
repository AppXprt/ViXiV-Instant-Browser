...
public class BrowserPresenter {
     ...
     */
    public void tabChangeOccurred(@Nullable InstantView tab) {
        mView.notifyTabViewChanged(mTabsModel.indexOfTab(tab));
    }

    private void onTabChanged(@Nullable InstantView newTab) {
    ...
    public void deleteTab(int position) {
        ...
        final InstantView tabToDelete = mTabsModel.getTabAtPosition(position);
        ...
        final InstantView currentTab = mTabsModel.getCurrentTab();
        ...
        final InstantView afterTab = mTabsModel.getCurrentTab();
...
//            }
    ...
    public void onNewIntent(@Nullable final Intent intent) {
        mTabsModel.doAfterInitialization(new Runnable() {
            ...
            public void run() {
                ...
                if (tabHashCode != 0) {
                    InstantView tab = mTabsModel.getTabForHashCode(tabHashCode);
                    if (tab != null) {
                        tab.loadUrl(url);
                    }
                } else if (url != null) {
                    if (URLUtil.isFileUrl(url)) {
                        mView.showBlockedLocalFileDialog(new DialogInterface.OnClickListener() {
                            ...
                            public void onClick(DialogInterface dialog, int which) {
                                newTab(url, true);
                                mShouldClose = true;
                                InstantView tab = mTabsModel.lastTab();
                                if (tab != null) {
    ...
    public synchronized void tabChanged(int position) {
        ...
        InstantView tab = mTabsModel.switchToTab(position);
        onTabChanged(tab);
    }
    ...
    public synchronized boolean newTab(@Nullable String url, boolean show) {
        ...
        InstantView startingTab = mTabsModel.newTab((Activity) mView, url, mIsIncognito);
        ...
        if (show) {
            InstantView tab = mTabsModel.switchToTab(mTabsModel.last());
            onTabChanged(tab);
        }
    ...
    public void findInPage(@NonNull String query) {
        final InstantView currentView = mTabsModel.getCurrentTab();
        if (currentView != null) {
