...
import net.vixiv.instant.browser.BookmarksView;
import net.vixiv.instant.browser.BrowserPresenter;
import net.vixiv.instant.browser.BrowserView;
import net.vixiv.instant.IncognitoActivity;
import net.vixiv.instant.browser.SearchBoxModel;
import net.vixiv.instant.browser.TabsManager;
import net.vixiv.instant.browser.TabsView;
...
import net.vixiv.instant.controller.UIController;
...
import net.vixiv.instant.interpolator.BezierDecelerateInterpolator;
import net.vixiv.instant.receiver.NetworkReceiver;
...
import net.vixiv.instant.settings.activity.SettingsActivity;
import net.vixiv.instant.utils.DrawableUtils;
import net.vixiv.instant.utils.IntentUtils;
import net.vixiv.instant.utils.Preconditions;
import net.vixiv.instant.utils.ProxyUtils;
import net.vixiv.instant.utils.ThemeUtils;
import net.vixiv.instant.utils.UrlUtils;
import net.vixiv.instant.utils.Utils;
import net.vixiv.instant.utils.WebUtils;
import net.vixiv.instant.view.Handlers;
import net.vixiv.instant.view.InstantView;
import net.vixiv.instant.view.SearchView;
...
public abstract class BrowserActivity extends ThemableBrowserActivity implements BrowserView, UIController, OnClickListener {
    ...
    @Inject InstantDialogBuilder mBookmarksDialogBuilder;
        ...
        public boolean onKey(View searchView, int keyCode, KeyEvent keyEvent) {
            ...
            switch (keyCode) {
                case KeyEvent.KEYCODE_ENTER:
                    ...
                    final InstantView currentView = mTabsManager.getCurrentTab();
                    if (currentView != null) {
...
//        mBrowserFrame.setLayoutTransition(new LayoutTransition());
    ...
    private final Runnable mLongPressBackRunnable = new Runnable() {
        @Override
        public void run() {
            final InstantView currentTab = mTabsManager.getCurrentTab();
            ...
            if (event.isCtrlPressed()) {
                switch (event.getKeyCode()) {
                    ...
                    case KeyEvent.KEYCODE_R:
                        // Refresh current tab
                        InstantView currentTab = mTabsManager.getCurrentTab();
                        if (currentTab != null) {
                            currentTab.reload();
                        }
                        return true;
    ...
    @Override
    public void tabChanged(InstantView tab) {
    ...
    /**
     * displays the WebView contained in the InstantView Also handles the
            ...
            .subscribe(new SingleOnSubscribe<String>() {
                ...
                public void onItem(@Nullable String item) {
                    ...
                    for (int i = 0; i < mTabsManager.size(); i++) {
                        InstantView instantView = mTabsManager.getTabAtPosition(i);
                        String url = instantView != null ? instantView.getUrl() : null;
    ...
    private void openDownloads() {
        new DownloadsPage().getDownloadsPage()
            ...
            .subscribe(new SingleOnSubscribe<String>() {
                ...
                public void onItem(@Nullable String item) {
                    Preconditions.checkNonNull(item);
                    InstantView view = mTabsManager.getCurrentTab();
                    if (view != null) {
                        view.loadUrl(item);
                    }
                }
    ...
    public synchronized void onCreateWindow(Message resultMsg) {
        ...
        if (newTab("", true)) {
            InstantView newTab = mTabsManager.getTabAtPosition(mTabsManager.size() - 1);
            if (newTab != null) {
    ...
    /**
     * Closes the specified {@link InstantView}. This implements
     ...
     *
     * @param view the InstantView to close, delete it.
     */
    @Override
    public void onCloseWindow(InstantView view) {
    ...
    @Override
    public void handleNewTab(@NonNull InstantDialogBuilder.NewTab newTabType, @NonNull String url) {
        mDrawerLayout.closeDrawers();
        switch (newTabType) {
            case FOREGROUND:
                newTab(url, true);
